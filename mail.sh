...
genMail() {
    curl --request GET \
        --url 'https://www.1secmail.com/api/v1/?action=genRandomMailbox&count=1' >/etc/tmpm/mail.txt
    generated=$(cat /etc/tmpm/mail.txt | tr -d '"[]')
    rm=$(echo $generated >/etc/tmpm/mail.txt)
    sa=$(echo $generated >>/etc/tmpm/recentmail.txt)
    local env_msg
    env_msg="━━━━━━━━━━━━━━━━━━━━━\n"
...
cgenMail() {
    ...
    env_msg+="<b> Your Generated Temp Mail : <code> $generated</code></b>\n"
    env_msg+="━━━━━━━━━━━━━━━━━━━━━\n\n"
    ShellBot.sendMessage --chat_id ${callback_query_message_chat_id[$id]} --text "$env_msg" \
        --reply_markup "$keyboard1" \
        --parse_mode html
    return 0
}
...
refMes() {
    log=$(awk -F\@ '{print $1}' /etc/tmpm/mail.txt)
    dom=$(awk -F\@ '{print $2}' /etc/tmpm/mail.txt)
    curl --request GET \
        --url "https://www.1secmail.com/api/v1/?action=getMessages&login=$log&domain=$dom" >/etc/tmpm/message.txt
    mess=$(cat /etc/tmpm/message.txt | tr -d '"[]{},')
    local env_msg
    env_msg="━━━━━━━━━━━━━━━━━━━━━\n"
...
ShellBot.regHandleFunction --function refMes --callback_data reffmess
ShellBot.regHandleFunction --function recMail --callback_data recma
ShellBot.regHandleFunction --function cgenMail --callback_data genma
unset keyboard1
